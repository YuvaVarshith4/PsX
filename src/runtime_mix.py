...
class Runtime:
    ...
    def execute(self, node):
        if node.type == 'VarDecl':
            ...
            if var_name in self.env:
                if op == '+=':
                    if isinstance(self.env[var_name], str) or isinstance(val, str):
                        self.env[var_name] = str(self.env[var_name]) + str(val)
                    else: self.env[var_name] += val
            ...
            var_name, op = node.value
            if var_name in self.env:
                if op == '++': self.env[var_name] += 1
                elif op == '--': self.env[var_name] -= 1
            ...
            if arr_name in self.env:
                ...
                if isinstance(arr, list):
                    if idx >= len(arr): arr.extend([None] * (idx - len(arr) + 1))
                    arr[idx] = val
            ...
            self.return_value = self.eval_expr(node.children[0]) if node.children else None
            raise ReturnException()
        elif node.type == 'Print':
            print(self.eval_expr(node.children[0]))
        elif node.type == 'If':
            if self.eval_expr(node.children[0]):
                for stmt in node.children[1].value: self.execute(stmt)
            elif len(node.children) > 2 and node.children[2].value:
                for stmt in node.children[2].value: self.execute(stmt)
            ...
            while True:
                ...
                if not self.eval_expr(node.children[1]): break
        elif node.type == 'For':
            ...
            end_range = end + 1 if step > 0 else (end - 1 if step < 0 else end)
    ...
    def call_function(self, func_name, args):
        ...
        if isinstance(func, NativeFunction): return func.func(evaluated_args)
        if not isinstance(func, Function): raise RuntimeError(f"'{func_name}' is not a function")
        
        old_env = self.env
        self.env = copy.deepcopy(func.closure_env)
        for i, param in enumerate(func.params):
            self.env[param] = evaluated_args[i] if i < len(evaluated_args) else None
        ...
        try:
            ...
            if func.is_arrow and getattr(func, 'is_expr', False):
                ...
                for stmt in func.body: self.execute(stmt)
    ...
    def eval_expr(self, node):
        ...
        if node.type == 'Value': return self.eval_value(node.value)
        elif node.type == 'ArrayLiteral': return [self.eval_expr(child) for child in node.children]
        elif node.type == 'ArrayIndex':
            arr = self.env.get(node.value)
            return arr[int(self.eval_expr(node.children[0]))]
        elif node.type == 'Call': return self.call_function(node.value, node.children)
            ...
            elif op in ('==', '!=', '>', '<', '>=', '<='): return self.compare(left, op, right)
